...
$(document).ready(function(){
  let audio,
      athkar = [];
  load(false, true);
  browser.storage.sync.get(["show_date", "date"]).then(result => {
    if(!result.hasOwnProperty("show_date") || result.show_date){
      ...
      if(!result.hasOwnProperty("date") || result.date.gregorianDate !== currentDate){
        $.get('http://api.aladhan.com/v1/gToH', function(data){
          ...
          browser.storage.sync.set({date: {gregorianDate: currentDate, hijriData: hijriData}});
  ...
  function load(reload, withTopSites){
    ...
    $(".reload img").hide();
    $(".reload .loader").show();
    browser.storage.local.get(['image', 'verse']).then(result => {
      browser.storage.sync.get(['show_translation', 'translation_language', 'recitation',
                                  ...
                                  .then(syncResult => {
                                    ...
                                    if(withTopSites && (!syncResult.hasOwnProperty('show_top_sites') || syncResult.show_top_sites)){
                                      browser.topSites.get(addTopSites);
                                    }
