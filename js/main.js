...
$(document).ready(function(){
  ...
  load(false, true);
  browser.storage.sync.get(["show_date"], function(result){
  ...
  function load(reload, withTopSites, isReport = false){
    ...
    browser.storage.local.get(['image', 'verse', 'calendar', 'prayerTimesCalendar'], function(result){
  ...
  function setCalendar (data) {
    ...
    for (let j = 0; j < currentHijriMonths.length; j++) {
      ...
      hijriMonthsStr += browser.i18n.getMessage(currentHijriMonths[j]);
    }
  ...
  function getPrayerTimesCalendar () {
    navigator.geolocation.getCurrentPosition((position) => {
      const date = new Date();
      browser.storage.sync.get(['prayer_times_method'], function (result) {
        ...
        '&month=' + (date.getMonth() + 1) + '&year=' + date.getFullYear() + '&method=' + (result.hasOwnProperty('prayer_times_method') ? result.prayer_times_method : 0), 
          ...
          browser.storage.local.set({prayerTimesCalendar: {month: date.getMonth(), calendar: data.data}}, function () {
            getPrayerTimes();
          });
        });
      })
  ...
  function getPrayerTimes() {
    browser.storage.local.get(['prayerTimesCalendar'], function(result) {
      console.log(result)
