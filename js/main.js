...
$(document).ready(function(){
  let audio,
      ...
      hijriHolidays = [],
      extensionURL = encodeURI("https://addons.mozilla.org/en-US/firefox/addon/quran-in-new-tab/");
  ...
  load(false, true);
  browser.storage.sync.get(["show_date", "date", "showed_survey_popup", 
    ...
    if (!result.hasOwnProperty("showed_survey_popup") || !result.showed_survey_popup) {
      Swal.fire({
        title: browser.i18n.getMessage('feedback_window_title'),
        html: browser.i18n.getMessage('feedback_window_content'),
        showConfirmButton: false,
        showCloseButton: true,
        cancelButtonText: browser.i18n.getMessage('cancel'),
        onClose: function () {
          browser.storage.sync.set({showed_survey_popup: true});
        }
      })
    }
    ...
    if (!result.hasOwnProperty("showed_new_feature_report") || !result.showed_new_feature_report) {
      Swal.fire({
        icon: 'info',
        title: browser.i18n.getMessage('reporting_window_message'),
        html: browser.i18n.getMessage('reporting_window_content'),
        showConfirmButton: false,
        showCloseButton: true,
        cancelButtonText: browser.i18n.getMessage('cancel'),
        onClose: function () {
          browser.storage.sync.set({showed_new_feature_report: true});
        }
      })
    }
    ...
    if (!result.hasOwnProperty("showed_new_feature_calendar") || !result.showed_new_feature_calendar) {
      Swal.fire({
        icon: 'info',
        title: browser.i18n.getMessage('new_features_title'),
        html: browser.i18n.getMessage('new_features_calendar_content'),
        showConfirmButton: false,
        showCloseButton: true,
        cancelButtonText: browser.i18n.getMessage('cancel'),
        customClass: {
        ...
        onClose: function () {
          browser.storage.sync.set({showed_new_feature_calendar: true});
        }
      })
    }
    ...
    if (!result.hasOwnProperty("showed_new_top_sites") || !result.showed_new_top_sites) {
      Swal.fire({
        icon: 'info',
        title: browser.i18n.getMessage('new_features_title'),
        html: browser.i18n.getMessage('new_feature_top_sites'),
        showConfirmButton: false,
        showCloseButton: true,
        cancelButtonText: browser.i18n.getMessage('cancel'),
        onClose: function () {
          browser.storage.sync.set({showed_new_top_sites: true});
        }
      })
    }

    if (!result.hasOwnProperty('showed_languages') || !result.showed_languages) {
      let currentLocale = browser.i18n.getUILanguage(),
          html = '';
      ...
      if (html.length) {
        Swal.fire({
          ...
          cancelButtonText: browser.i18n.getMessage('cancel'),
          onClose: function () {
            browser.storage.sync.set({showed_languages: true});
          }
        });
      } else {
        browser.storage.sync.set({showed_languages: true});
  ...
  $(".audio-player").click(function(){
    ...
    if(!audio){
      browser.storage.local.get(["verse"], function(result){
  ...
  $(".report").click(function() {
    Swal.fire({
      title: browser.i18n.getMessage('report_image_window_title'),
      html: browser.i18n.getMessage('report_image_window_content'),
      showConfirmButton: true,
      showCloseButton: true,
      confirmButtonText: browser.i18n.getMessage('report_image_window_title'),
  ...
  function load(reload, withTopSites, isReport = false){
    ...
    browser.storage.local.get(['image', 'verse', 'calendar'], function(result){
      browser.storage.sync.get(['show_translation', 'translation_language', 'recitation',
          ...
          if(result.hasOwnProperty('verse') && result.verse && now <= result.verse.timeout && !reload){
            ...
            $.get(url, function(data){
              if(data.data){
                ...
                browser.storage.local.set({verse});
              }
            }).fail(function(){
              $(".translation-container").remove();
  ...
  $("body").on("click", ".top-sites-container a .remove", function (e) {
    ...
    Swal.fire({
      title: browser.i18n.getMessage('remove_top_site_title'),
      html: browser.i18n.getMessage('remove_top_site_content'),
      showConfirmButton: true,
      confirmButtonText: browser.i18n.getMessage('remove'),
      showCloseButton: true,
      showCancelButton: true,
      cancelButtonText: browser.i18n.getMessage('cancel'),
      icon: 'warning',
      showLoaderOnConfirm: true,
      preConfirm: function () {
        browser.storage.sync.get(['removed_top_sites'], function (storageSync) {
          ...
          if (removedTopSites.indexOf(removeUrl) == -1) {
            ...
            browser.storage.sync.set({removed_top_sites: removedTopSites}, function () {
              parent.slideUp();
              setTimeout(function () {
                parent.remove();
              }, 2000);
  ...
  function setDates(dateObj, hijriData){
    ...
    $(".hijri-date").text(hijriData.getDate() + " " + browser.i18n.getMessage(hijriMonths[hijriData.getMonth() - 1]) + " " + hijriData.getFullYear());
  }
        ...
        function(json, textStatus) {
          if (json.success) {
            if (json.blacklisted) {
              ...
              let timeout = calculateTimeout();
              browser.storage.local.set({image: {src: xhr.responseURL, timeout}});
            }
          }
        });
      },
  ...
  function getNewCalendar () {
    ...
    browser.storage.local.set({calendar: {date: currentDate.toString(), data: calendarData, hijriHolidays: hijriHolidays}});
  }
  ...
  function setCalendar (data) {
    ...
    $(".calendar__header").children().each(function (index) {
      $(this).text(browser.i18n.getMessage(weekdays[index]));
    });
    ...
    $("#gregorianMonth").text(browser.i18n.getMessage(data[0].gregorian.month.en));
    $("#hijriMonth").text(browser.i18n.getMessage(data[0].hijri.month.en));
    ...
    for (let j = 0; j < 7; j++) {
      ...
      if (i <= totalDays && i > 0) {
        ...
        if (isFastingDay(parseInt(calendarData[i- 1].hijri.day), originalWeekdays[j], hijriHolidays, 
          ...
          dayStr += '<span class="badge badge-danger calendar-note">' + browser.i18n.getMessage('Fasting') + '</span>';
        }
      }
